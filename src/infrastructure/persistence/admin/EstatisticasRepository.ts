...
export class EstatisticasRepository {
  ...
  async obterPedidosPorPeriodo(
    ...
    const records = await this.db.pedidos
      .where('restauranteId')
   ...
   */
  async obterItensPopulares(
    restauranteId: string,
    periodo: string
  ): Promise<EstatisticasItem[]> {
  ...
  private parseItens(itensJson: string): Array<{
    ...
    return itens.map((item: { produtoId: string; nome: string; quantidade: number; subtotal: { valor: number } }) => ({
