...
const mockConfiguracoesTable = {
  ...
  delete: vi.fn(async (id: string) => { mockConfiguracoes.delete(id); }),
};
    ...
    equals: vi.fn((value: unknown) => ({
      first: vi.fn(async () => {
        for (const record of mockUserRestaurants.values()) {
          if ((record as Record<string, unknown>)[Object.keys(record).find(k => k === 'user_id') || 'user_id'] === value) {
            return record;
          }
        }
        return undefined;
      ...
      toArray: vi.fn(async () => {
        return Array.from(mockUserRestaurants.values()).filter(
          (r) => (r as Record<string, unknown>)[Object.keys(r).find(k => k === 'user_id') || 'user_id'] === value
        );
      }),
    })),
    anyOf: vi.fn(() => ({
...
describe('RestauranteRepository', () => {
  ...
  const criarRestaurante = (overrides: Partial<{
    id: string;
    nome: string;
    cnpj: string;
    endereco: string;
