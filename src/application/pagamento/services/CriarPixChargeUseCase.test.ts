...
describe('CriarPixChargeUseCase', () => {
  ...
  const mockPedido = {
    ...
    itens: [{ id: 'item-1', produtoId: 'prod-1', nome: 'X-Burger', precoUnitario: { valor: 5000, moeda: 'BRL' }, quantidade: 1, modificadoresSelecionados: [], pedirObservacao: '' }],
  ...
  describe('execute', () => {
    it('deve lançar erro quando pedido não existe', async () => {
      ...
      await expect(useCase.execute({ pedidoId: 'pedido-inexistente' }))
