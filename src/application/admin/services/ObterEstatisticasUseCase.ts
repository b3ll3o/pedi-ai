...
export class ObterEstatisticasUseCase implements UseCase<EstatisticasInput, Estatisticas> {
  ...
  async execute(input: EstatisticasInput): Promise<Estatisticas> {
    // Calcular período
    const { dataInicio, dataFim } = this.calcularPeriodo(input.periodo, input.dataInicio, input.dataFim);

    // Buscar pedidos do período
    const todosPedidos = await this.pedidoRepo.findByRestauranteId(input.restauranteId);
    const pedidosPeriodo = todosPedidos.filter(p => p.createdAt >= dataInicio && p.createdAt <= dataFim);
    ...
    const ticketMedio = totalPagos > 0
      ? totalFaturamento.multiplicar(1 / totalPagos)
      : Dinheiro.ZERO;
