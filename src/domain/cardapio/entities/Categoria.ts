...
export class Categoria extends EntityClass<CategoriaProps> {
  ...
  atualizarOrdem(novaOrdem: number): void {
    Object.assign(this.props, { ordemExibicao: novaOrdem, atualizadoEm: new Date(), version: this.props.version + 1 });
  }
  ...
  static criar(props: Omit<CategoriaProps, 'id' | 'criadoEm' | 'atualizadoEm' | 'deletedAt' | 'version'>): Categoria {
    const now = new Date();
