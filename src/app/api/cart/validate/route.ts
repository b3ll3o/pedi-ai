...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isDevDatabase()) {
      return handleDevValidateCart(items, restaurantId, tableId, errors)
    } else {
      return handleSupabaseValidateCart(items, restaurantId, tableId, errors)
    }
  } catch (error) {
...
async function handleDevValidateCart(
  items: CartItem[],
  restaurantId: string,
  ...
  const productModifierGroupsData = db
    ...
    .where(inArray(schema.productModifierGroups.product_id, productIds))
    .all()
  ...
  const modifierGroups = modifierGroupIds.length > 0
    ? db
        .select({
          id: schema.modifierGroups.id,
          name: schema.modifierGroups.name,
          required: schema.modifierGroups.required,
          min_selections: schema.modifierGroups.min_selections,
        })
        .from(schema.modifierGroups)
        .where(and(
          inArray(schema.modifierGroups.id, modifierGroupIds),
          eq(schema.modifierGroups.required, true)
        ))
        .all()
    : []
  ...
  if (tableId) {
    const table = db
      .select({ id: schema.tables.id, active: schema.tables.active })
      .from(schema.tables)
      .where(and(
        eq(schema.tables.id, tableId),
        eq(schema.tables.restaurant_id, restaurantId),
        eq(schema.tables.active, true)
      ))
      .get()
...
async function handleSupabaseValidateCart(
  items: CartItem[],
  restaurantId: string,
  tableId: string | undefined,
  errors: ValidationError[]
