  ...
  try {
    ...
    if (isDevDatabase()) {
      const table = await db
        .select()
        .from(tables)
        .where(and(eq(tables.id, id), eq(tables.restaurant_id, restaurantId), isNull(tables.deleted_at)))
        .limit(1)
        .get();
    ...
    if (isDevDatabase()) {
      ...
      if (number !== undefined && number !== existingTable.number) {
        const conflict = await db
          .select({ id: tables.id })
          .from(tables)
          .where(and(eq(tables.restaurant_id, existingTable.restaurant_id), eq(tables.number, number), isNull(tables.deleted_at)))
          .limit(1)
          .get();

        if (conflict) {
          return NextResponse.json({ error: 'Table number already exists for this restaurant' }, { status: 409 });
        }
      }
      ...
      await db.update(tables).set(updateData as any).where(eq(tables.id, id));
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (isDevDatabase()) {
      ...
      // Soft delete
      await db.update(tables).set({ deleted_at: new Date().toISOString() }).where(eq(tables.id, id));
