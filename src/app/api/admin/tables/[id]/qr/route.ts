...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (isDevDatabase()) {
      ...
      if (!table.active) {
        return NextResponse.json({ error: 'Cannot generate QR code for inactive table' }, { status: 400 });
      }
    ...
    const qrPayload = generateQRPayload(table.restaurant_id as string, table.id as string, secretKey);
    const qrData = JSON.stringify({
      restaurant_id: table.restaurant_id,
      table_id: table.id,
      timestamp: qrPayload.timestamp,
