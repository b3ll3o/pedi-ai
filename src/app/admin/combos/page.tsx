...
export default function CombosPage() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setProducts(data.products?.map((p: products) => ({ id: p.id, name: p.name, price: p.price })) || []);
      }
    } catch {
  ...
  const handleToggleAvailability = useCallback(async (combo: ComboWithItems) => {
    try {
  ...
  const handleSubmit = useCallback(async (data: ComboInput) => {
