  ...
  describe('route.ts code verification', () => {
    it('o código em route.ts deve usar role: "dono" (não "owner")', async () => {
      ...
      const routeContent = await import('fs').then(fs => 
        fs.readFileSync('/home/leo/Documentos/projetos/pedi-ai/src/app/api/admin/restaurants/route.ts', 'utf-8')
      )
    ...
    it('o código em [id]/route.ts deve usar role: "dono" (não "owner")', async () => {
      ...
      const routeContent = await import('fs').then(fs => 
        fs.readFileSync('/home/leo/Documentos/projetos/pedi-ai/src/app/api/admin/users/[id]/route.ts', 'utf-8')
      )
...
describe('Enum user_role migration 0021 compliance', () => {
  ...
  it('sistema deve funcionar corretamente com valores pt-BR', () => {
    ...
    // All should be valid
    const allRoles = [restaurantOwnerRole, restaurantManagerRole, restaurantAttendantRole, restaurantCustomerRole]
