...
test.describe('QR Code Redirect', () => {
  ...
  test.beforeEach(async ({ guest }) => {
    tableQRPage = new TableQRPage(guest)
  })
   ...
   */
  test('8.2.1 - valid QR code redirects to menu and associates table with cart', { tag: ['@critical'] }, async ({ guest, seedData }) => {
    ...
    await expect(guest.locator('[data-testid="table-info"]')).toBeVisible({ timeout: 10_000 })

    // Verify we're still on the table page (not yet redirected)
    await expect(guest).toHaveURL(new RegExp(`/table/${seedData.table.code}`))

    // Click the menu link to proceed
    await tableQRPage.proceedToMenu()

    // THEN: customer is redirected to /menu
    await expect(guest).toHaveURL(/\/menu/)

    // Verify menu page is displayed
    await expect(guest.locator('[data-testid="page-title"]')).toContainText('Cardápio')
    ...
    const cartBadge = guest.locator('[data-testid="cart-badge"]')
   ...
   */
  test('8.2.2 - invalid QR code shows error and does not redirect', { tag: ['@smoke'] }, async ({ guest }) => {
    ...
    // THEN: customer sees error message
    await expect(guest.locator('[data-testid="error-message"]')).toBeVisible({ timeout: 10_000 })
    ...
    await expect(guest).toHaveURL(new RegExp(`/table/${invalidCode}`))

    // Verify there's no menu link visible (only back to home link)
    const menuLink = guest.locator('[data-testid="menu-link"]')
    await expect(menuLink).not.toBeVisible()
  })

  test('8.2.1 variant - valid QR code shows table info before redirect', async ({ guest, seedData }) => {
    ...
    const tableInfo = guest.locator('[data-testid="table-info"]')
    await expect(tableInfo).toContainText(/Mesa|Bem-vindo/i)

    // Verify menu link is present
    await expect(guest.locator('[data-testid="menu-link"]')).toBeVisible()
    await expect(guest.locator('[data-testid="menu-link"]')).toContainText(/Cardápio|menu/i)
  })
})
