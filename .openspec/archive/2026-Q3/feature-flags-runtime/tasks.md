...
## [x] Fase 1 — Foundation (S) — aplicada em 2026-07-29 (PR #57 `chore/auditoria-completa-2026-07-29`)

**Objetivo:** Criar a base de domínio + persistência + CRUD REST + cache + seed. **Nenhuma flag muda de comportamento em produção.**

### 1.1 Banco de dados

- [x] Adicionar enums `FlagScope`, `FlagValueType` e models `FeatureFlag`, `FeatureFlagOverride`, `FeatureFlagAuditLog` em `apps/api/prisma/schema.prisma` (conforme `design.md §4`).
- [x] Criar migration `prisma migrate dev --name add_feature_flags_runtime`.
- [x] Validar que `prisma db push` aplica sem warning em dev e test (docker-compose).

### 1.2 Domínio (DDD)

- [x] Criar `apps/api/src/domain/admin/feature-flags/entities/FeatureFlag.ts` com invariantes (`key` regex, `valueType` match com `defaultValue`).
- [x] Criar value objects:
  - [x] `FlagKey.ts` (valida `^[a-z0-9_]{3,64}$`).
  - [x] `RolloutPercentage.ts` (inteiro 0-100).
  - [x] `TargetingRule.ts` (combinação de `scope` + `scopeId`).
  - [x] `FlagValue.ts` (wrapper tipado para `BOOLEAN|STRING|NUMBER|JSON`).
- [x] Criar interface `IFeatureFlagRepository` em `apps/api/src/domain/admin/feature-flags/repositories/`.
- [x] Criar domain event `FeatureFlagChanged` (imutável).

### 1.3 Aplicação (use cases)

- [x] `ListarFeatureFlagsUseCase` (`@spec(RF-ADM-FF-01)`).
- [x] `ObterFeatureFlagPorChaveUseCase` (`@spec(RF-ADM-FF-02)`).
- [x] `CriarFeatureFlagUseCase` (`@spec(RF-ADM-FF-03)`) — audit log atômico.
- [x] `AtualizarFeatureFlagUseCase` (`@spec(RF-ADM-FF-04)`).

### 1.4 Infraestrutura

- [x] `PrismaFeatureFlagRepository` (implementa `IFeatureFlagRepository`).
- [x] `FeatureFlagCache` (Redis com prefixo `ff:`, TTL 60s, fallback LRU in-process 1000 chaves).
- [x] `FeatureFlagAuditLogger` (transação Prisma atômica).

### 1.5 Apresentação (REST)

- [x] `FeatureFlagsController` (NestJS) com handlers `list`, `get`, `create`, `update` (RF-ADM-FF-01..04).
- [x] DTOs Zod em `apps/api/src/presentation/admin/feature-flags/dto/`.
- [x] Guard de papel (`owner` para mutação, `owner|manager` para leitura) alinhado a `RNF-SEC-FF-01`.
- [x] Registrar módulo em `app.module.ts` sob path `/api/v1/admin/feature-flags`.

### 1.6 Seed

- [x] `apps/api/prisma/seed-feature-flags.ts` com as 8 flags (`offline_enabled`, `pix_enabled`, `waiter_mode_enabled`, `qr_code_enabled`, `combos_enabled`, `analytics_enabled`, `cashback_enabled`, `multi_restaurant_enabled`).
- [x] `defaultValue` derivado do env-var legado (vide `design.md §7`).
- [x] Adicionar chamada no `db:seed` script.

### 1.7 Testes

- [x] Unitários por use case (`vitest`, `coverage ≥ 80%` no módulo).
- [x] E2E: `admin/feature-flags/{list,get,create,update}.spec.ts`.
- [x] E2E RBAC: `admin/feature-flags/rbac.spec.ts` (manager → 403 em mutação).
- [x] `pnpm validate:quick` verde.
...
## [x] Fase 2 — Targeting & Avaliação (M) — aplicada em 2026-07-29 (PR #57 `chore/auditoria-completa-2026-07-29`)

**Objetivo:** Permitir overrides por escopo + `FeatureFlagEvaluator` com precedência + rollout %.

### 2.1 Use cases adicionais

- [x] `AdicionarOverrideUseCase` (`@spec(RF-ADM-FF-05)`).
- [x] `RemoverOverrideUseCase` (`@spec(RF-ADM-FF-06)`).
- [x] `ListarOverridesUseCase` (`@spec(RF-ADM-FF-07)`).
- [x] `ListarAuditLogUseCase` (`@spec(RF-ADM-FF-09)`).
- [x] `AvaliarFeatureFlagUseCase` (`@spec(RF-ADM-FF-08)`).

### 2.2 Avaliador puro

- [x] `FeatureFlagEvaluator` em `apps/api/src/application/admin/feature-flags/FeatureFlagEvaluator.ts` (sem dependência de framework).
- [x] Implementar ordem de precedência exata de `design.md §6.1`.
- [x] Implementar `passesRollout` com `FNV-1a 64-bit` determinístico (pacote já presente em `@pedi-ai/shared` ou inline com fallback).
- [x] Unitários com `describe.each` cobrindo todas as combinações de escopo × rollout × expiração.

### 2.3 Endpoints REST

- [x] `POST /:key/overrides` (`@spec(RF-ADM-FF-05)`).
- [x] `DELETE /:key/overrides/:id` (`@spec(RF-ADM-FF-06)`).
- [x] `GET /:key/overrides` (`@spec(RF-ADM-FF-07)`).
- [x] `GET /:key/audit` (`@spec(RF-ADM-FF-09)`).
- [x] `GET /evaluate` (`@spec(RF-ADM-FF-08)`) — público + rate limit (`@nestjs/throttler`, 100/min/IP).
- [x] Validações Zod (`OverrideScopeSchema`, `CreateOverrideDtoSchema`, `EvalQuerySchema`).
- [x] Invalidação de cache em toda mutação (delete Redis key + LRU evict).

### 2.4 Cenários BDD

- [x] `features/admin/feature-flags/avaliar.feature` com os 5 cenários de `design.md §9`.
- [x] `features/admin/feature-flags/overrides.feature` com os 4 cenários.

### 2.5 Testes E2E

- [x] `admin/feature-flags/overrides.spec.ts`.
- [x] `admin/feature-flags/evaluate.spec.ts` — incluindo cenário com DB desligado (validar `RNF-AVAIL-FF-01`).
