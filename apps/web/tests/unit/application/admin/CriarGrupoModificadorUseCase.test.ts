...
describe('CriarGrupoModificadorUseCase', () => {
  ...
  describe('execute', () => {
    it('deve criar grupo de modificador com dados válidos', async () => {
      ...
      const { CriarGrupoModificadorUseCase } =
        await import('@/application/admin/services/CriarGrupoModificadorUseCase');
      const useCase = new CriarGrupoModificadorUseCase(mockGrupoRepo, mockRestauranteRepo);
