...
describe('AlterarStatusPedidoUseCase', () => {
  ...
  describe('transições inválidas', () => {
    ...
    it('não deve permitir transição de received para paid', async () => {
      ...
      await expect(useCase.execute({ pedidoId: 'pedido-123', novoStatus: 'paid' })).rejects.toThrow(
        /Transição de status/
      );
    });
  ...
  describe('erros', () => {
    it('deve lançar erro quando pedido não existe', async () => {
      ...
      await expect(useCase.execute({ pedidoId: 'nao-existe', novoStatus: 'paid' })).rejects.toThrow(
