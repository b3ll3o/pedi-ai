...
describe('analyticsService', () => {
  ...
  describe('getItensMaisVendidos', () => {
    ...
    it('lança erro genérico quando response.ok false e json rejeita', async () => {
      ...
      await expect(getItensMaisVendidos('rest-123')).rejects.toThrow(
        'Erro ao buscar itens mais vendidos'
      );
    });
  });
  ...
  describe('getPedidosPorPeriodo', () => {
    ...
    it('lança erro quando response.ok é false', async () => {
      ...
      await expect(getPedidosPorPeriodo('rest-123', { period: 'day' })).rejects.toThrow(
        'Erro interno'
      );
    });
    ...
    it('lança erro genérico quando response.ok false e json rejeita', async () => {
      ...
      await expect(getPedidosPorPeriodo('rest-123', { period: 'day' })).rejects.toThrow(
        'Erro ao buscar pedidos por período'
      );
    });
  });
