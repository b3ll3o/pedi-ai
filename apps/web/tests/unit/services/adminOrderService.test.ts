  ...
  describe('updateOrderStatus', () => {
    it('faz requisição PATCH para /api/admin/orders/{orderId}/status', async () => {
      mockNextResponse({
        ok: true,
        json: () =>
          Promise.resolve({ id: 'o1', status: 'paid', payment_status: 'pending', updated_at: '' }),
      });
    ...
    it('define header Content-Type como application/json', async () => {
      mockNextResponse({
        ok: true,
        json: () =>
          Promise.resolve({ id: 'o1', status: 'ready', payment_status: 'pending', updated_at: '' }),
      });
    ...
    it('lanza erro genérico quando response ok=false sem mensagem', async () => {
      ...
      await expect(updateOrderStatus('order-123', 'pending')).rejects.toThrow(
