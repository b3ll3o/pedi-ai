...
test.describe('Admin Analytics', () => {
  ...
  test.describe('View popular items report', () => {
    test.skip(
      'should display top 10 products when selecting Produtos tab',
      { tag: '@smoke' },
      async ({ admin: _admin }) => {
        await analyticsPage.selectProductsTab();
        await expect(analyticsPage.popularItemsTable).toBeVisible();
    ...
    // SKIP: Requires existing order data
    test.skip('should display popular items with product name, quantity and revenue', async ({
      admin: _admin,
    }) => {
      await analyticsPage.selectProductsTab();
    ...
    // SKIP: Requires existing order data
    test.skip('should show empty state when no product data available', async ({
      admin: _admin,
    }) => {
      await analyticsPage.selectProductsTab();
  ...
  test.describe('View orders per period', () => {
    test.skip(
      'should display orders grouped by day when selecting date range',
      { tag: '@smoke' },
      async ({ admin: _admin }) => {
        await analyticsPage.selectOverviewTab();
    ...
    test.skip(
      'should calculate total revenue correctly for selected period',
      { tag: '@smoke' },
      async ({ admin: _admin }) => {
        await analyticsPage.selectOverviewTab();
