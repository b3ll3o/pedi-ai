...
test.describe('Admin Modifier Groups', () => {
  ...
  test.describe('5.2.1 - Create modifier group', () => {
    test('should create modifier group with name, required flag, and values', async ({
      admin: _admin,
    }) => {
      ...
      await expect(
        modifierGroupsPage.modifierGroupsList
          .filter({ hasText: `${groupName}` })
          .filter({ hasText: 'Obrigatório' })
      ).toBeVisible();
    });
  ...
  test.describe('5.2.2 - Edit modifier group', () => {
    test('should update modifier group name', async ({ admin: _admin }) => {
      ...
      const oldGroupCount = await modifierGroupsPage.modifierGroupsList
  ...
  test.describe('5.2.3 - Delete modifier group', () => {
    test('should soft-delete modifier group', async ({ admin: _admin }) => {
      ...
      const count = await modifierGroupsPage.modifierGroupsList
        .filter({ hasText: groupName })
        .count();
      expect(count).toBe(0);
    });
  ...
  test.describe('5.2.6 - Remove modifier value', () => {
    test('should soft-delete modifier value from group', async ({ admin: _admin }) => {
      ...
      const removedValueCount = await modifierGroupsPage.valuesSection
        .filter({ hasText: 'Valor para Remover' })
        .count();
      expect(removedValueCount).toBe(0);

      // Verifica que os outros valores ainda existem
      await expect(modifierGroupsPage.valuesSection.filter({ hasText: 'Valor 1' })).toBeVisible();
