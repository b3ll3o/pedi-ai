 ...
 */
export async function captureCartSummary(page: Page): Promise<{
  itemCount: number;
  subtotal: string;
  tax: string;
  total: string;
 ...
 */
export async function captureA11ySnapshot(page: Page): Promise<{
  tree: unknown;
  violations: string[];
}> {
  const _snapshot = await page.accessibility.snapshot();
  ...
  const buttonsWithoutName = (
    html.match(/<button(?!.*(aria-label|text))[^-]*>[\s]*<\/button>/gi) || []
  ).length;
