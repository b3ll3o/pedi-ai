...
const SEED_RESULT_PATH = path.join(
  __dirname,
  '..',
  '..',
  '..',
...
function loadSeedData(): SeedData {
  if (!fs.existsSync(SEED_RESULT_PATH)) {
    throw new Error(
      `Seed result not found at ${SEED_RESULT_PATH}. Run seed first: pnpm test:e2e:seed`
    );
  }
...
async function _waitForOnlineToast(
...
test.describe('Offline Flow — PED-5', () => {
  ...
  test('Pedidos enfileirados são processados ao restaurar conexão', async ({ browser }) => {
    ...
    await expect(page.locator('[data-testid="online-indicator"]')).toBeVisible({ timeout: 8000 });
  ...
  test('Indicador visual de conectividade: banner offline e toast de restauração', async ({
    browser,
  }) => {
    const seedData = loadSeedData();
    const context = await createAuthenticatedContext(browser, seedData.customer.email);
