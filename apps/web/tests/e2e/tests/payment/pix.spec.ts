...
test.describe('Pagamento Pix', () => {
  ...
  test.describe('Fluxo completo de pagamento Pix', () => {
    test('deve exibir QR code Pix ao confirmar pedido com pagamento Pix', async ({
      authenticated,
      seedData,
    }) => {
    ...
    test(
      'deve confirmar pagamento Pix via polling e atualizar status do pedido',
      {
        tag: ['@slow', '@pix'],
      },
  ...
  test.describe('Timeout de pagamento Pix', () => {
    test(
      'deve exibir mensagem de timeout quando pagamento Pix expira',
      {
        tag: ['@slow', '@pix'],
      },
    ...
    test(
      'deve permitir cancelar pedido após timeout de pagamento',
      {
        tag: ['@slow', '@pix'],
      },
      async ({ authenticated, seedData }) => {
