...
test.describe('Checkout', () => {
  ...
  test.skip(
    'should submit order with valid data',
    { tag: ['@smoke', '@slow'] },
    async ({ authenticated }) => {
  ...
  test('should display validation error when mandatory modifier not selected', async ({
    authenticated,
  }) => {
    await menuPage.goto();
    ...
    await checkoutPage.submitOrder();
    const errorVisible = await authenticated
      .locator('[data-testid="modifier-required-error"]')
      .isVisible();
