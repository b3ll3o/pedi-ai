...
test.describe('Restaurants - Navegação Pública', () => {
  ...
  test(
    'deve carregar e exibir restaurantes do seed',
    { tag: '@critical' },
    async ({ seedData }) => {
      await restaurantsPage.goto();
  ...
  test(
    'deve navegar para cardápio ao clicar em restaurante',
    { tag: ['@smoke', '@critical'] },
    async ({ seedData }) => {
      await restaurantsPage.goto();
      await restaurantsPage.waitForList();
  ...
  test(
    'deve exibir página de cardápio com título',
    { tag: ['@smoke', '@critical'] },
    async ({ seedData }) => {
  ...
  test(
    ...
    async ({ seedData }) => {
      ...
      await expect(restaurantsPage.getCardByName(seedData.restaurant.name)).toBeVisible();
    }
  );
