/**
 * Cobertura E2E: RF-TABLE-03 (Validar QR code)
...
test.describe('QR Code Redirect', () => {
   ...
   */
  test.skip(
    '8.2.1 - valid QR code redirects to menu and associates table with cart',
    { tag: ['@critical'] },
    async ({ authenticated, seedData }) => {
      ...
      await expect(authenticated.locator('[data-testid="table-info"]')).toBeVisible({
        timeout: 10_000,
      });
   ...
   */
  test(
    '8.2.2 - invalid QR code shows error and does not redirect',
    { tag: ['@smoke'] },
    async ({ authenticated }) => {
      ...
      // THEN: customer sees error message
      await expect(authenticated.locator('[data-testid="error-message"]')).toBeVisible({
        timeout: 10_000,
      });
  ...
  test('8.2.1 variant - valid QR code shows table info before redirect', async ({
    authenticated,
    seedData,
  }) => {
    ...
    await expect(authenticated.locator('[data-testid="menu-link"]')).toContainText(
      /Cardápio|menu/i
    );
  });
});
