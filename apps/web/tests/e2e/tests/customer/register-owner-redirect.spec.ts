...
test.describe('Registro com intent gerenciar_restaurante', () => {
  ...
  test.skip(
    'deve redirecionar para /admin/restaurants/new após registrar com intent gerenciar_restaurante',
    { tag: ['@critical'] },
    async ({ page }) => {
      ...
      const ownerIntentButton = page
        .locator('button[aria-pressed="true"]')
        .filter({ hasText: /gerenciar meu restaurante/i })
      ...
      await expect(page.locator('text=Conta criada com sucesso'))
      ...
      await page.waitForURL(/\/(menu|admin\/restaurants\/new|admin\/dashboard)/, {
        timeout: 10_000,
      });
  ...
  test(
    'deve manter intent "gerenciar_restaurante" após registro',
    { tag: ['@smoke'] },
    async ({ page }) => {
      const uniqueEmail = createUniqueEmail();
      const password = 'SenhaForte123!';
