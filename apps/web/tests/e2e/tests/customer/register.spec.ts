...
test.describe('Registro do Cliente', () => {
  ...
  test(
    'deve registrar novo cliente com credenciais válidas e redirecionar para login',
    { tag: ['@critical'] },
    async ({ page }) => {
  ...
  test(
    'deve fazer login com email recém-cadastrado e redirecionar para /menu',
    { tag: ['@critical'] },
    async ({ page, seedData }) => {
      ...
      const currentUrl = page.url();
      expect(currentUrl.includes('/menu') || currentUrl.includes('/login')).toBeTruthy();
    }
  );

  test(
    'deve exibir erro com email já existente',
    { tag: ['@critical'] },
    async ({ page, seedData }) => {
      // Tenta registrar com email que já existe
      await registerPage.register(
        'Novo Cliente',
        seedData.customer.email,
        'SenhaForte123!',
        'SenhaForte123!'
      );
  ...
  test('deve exibir erro com senhas que não coincidem', async ({ page }) => {
    ...
    const confirmPasswordField = page
      ...
      .locator('..')
      .locator('[data-testid="field-error"]');
  ...
  test(
    'deve navegar para /register ao clicar em "Começar Agora" na home',
    { tag: ['@smoke'] },
    async ({ page }) => {
      await page.goto('/');
  ...
  test.describe('Loading states', () => {
    test(
      'mostra formulário em até 10s quando API de sessão está lenta (timeout de 8s)',
      { tag: ['@resilience'] },
      async ({ page }) => {
        // Simular API de sessão com delay de 30s — o timeout de 8s no getSession
        // deve fazer a página exibir o formulário em vez de carregar infinitamente
        ...
        // O formulário deve aparecer em até 10s (8s timeout + 2s buffer)
    ...
    test(
      ...
      { tag: ['@resilience'] },
      async ({ page }) => {
        // Abortar todas as requisições para /api/auth/session — simula network error
        await page.route('/api/auth/session', async (route) => {
    ...
    test(
      ...
      async ({ page }) => {
        ...
        // Deve aparecer rapidamente (sem esperar timeout)
        await expect(registerPage.nameInput).toBeVisible({ timeout: 3000 });
        await expect(registerPage.emailInput).toBeVisible();
        await expect(registerPage.registerButton).toBeVisible();
      }
    );
