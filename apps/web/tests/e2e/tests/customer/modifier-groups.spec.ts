...
test.describe('Modifier Groups', () => {
  ...
  test('should not add product with required modifier without selection', async ({
    authenticated,
  }) => {
    ...
    await expect(
      authenticated.locator('[data-testid="modifier-error"], [data-testid="validation-error"]')
    ).toBeVisible();
  });

  test('should add product with required modifier when selection is made', async ({
    authenticated,
  }) => {
  ...
  test('should add product with optional modifier without selection', async ({ authenticated }) => {
    ...
    const requiredModifier = authenticated
      .locator('[data-testid^="modifier-option-"][data-required="true"]')
      .first();
  ...
  test('should adjust price with optional modifier selection', async ({ authenticated }) => {
    ...
    // Obter preço base
    const basePriceText = await authenticated
    ...
    const optionalModifier = authenticated
      .locator('[data-testid^="modifier-option-"][data-required="false"]')
      .first();
    ...
    const updatedPriceText = await authenticated
      .locator('[data-testid="total-with-modifiers"]')
      .textContent();
    const updatedPrice = parseFloat(
      updatedPriceText?.replace(/[^\d,]/g, '').replace(',', '.') ?? '0'
    );
