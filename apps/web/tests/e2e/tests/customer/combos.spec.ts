/**
 * Cobertura E2E: RF-MENU-07 (CRUD combo)
...
test.describe('Combos', () => {
  ...
  test('should display bundle price instead of sum', async ({ authenticated }) => {
    ...
    const bundlePriceLocator = authenticated.locator(
      '[data-testid="bundle-price"], [data-testid="product-price"]'
    );
    ...
    const bundlePrice = parseFloat(
      bundlePriceText?.replace(/[^\d,]/g, '').replace(',', '.') ?? '0'
    );
  ...
  test('should use bundle price in cart total', async ({ authenticated: _authenticated }) => {
    ...
    const itemPriceText = await cartPage.cartItems
      .first()
