...
test.describe('Autenticação do Cliente', () => {
  ...
  test(
    'deve logar com credenciais válidas de cliente',
    { tag: ['@smoke', '@critical'] },
    async ({ page, seedData }) => {
  ...
  test(
    'deve redirecionar para login ao acessar rota protegida',
    { tag: ['@smoke', '@critical'] },
    async ({ page }) => {
  ...
  test('admin logado via /login deve ir para /menu (cliente redirecionado)', async ({
    page,
    seedData,
  }) => {
  ...
  test('deve exibir erro com email inexistente na recuperação de senha', async ({
    page: _page,
  }) => {
