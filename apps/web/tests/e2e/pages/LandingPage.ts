...
export class LandingPage {
  ...
  constructor(page: Page) {
    ...
    this.funcionalidadesLink = page
      .locator('nav[aria-label="Navegação principal"] a[href="#features"]')
      .first();
    this.precosLink = page
      .locator('nav[aria-label="Navegação principal"] a[href="#pricing"]')
      .first();
    this.ctaButton = page
      .locator(
        'nav[aria-label="Navegação principal"] a[href="/register"]:has-text("Começar Agora")'
      )
      .first();
  ...
  async getLinkHref(linkText: string): Promise<string | null> {
    return this.page
      .locator(`nav[aria-label="Navegação principal"] a:has-text("${linkText}")`)
      .getAttribute('href');
  }
