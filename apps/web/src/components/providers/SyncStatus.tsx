    ...
    <div className="sync-status" data-testid="offline-sync-status">
      {hasQueue && (
        <div className="sync-queue" data-testid="offline-queue-indicator">
          ...
          {totalPending > 0 && <span className="sync-pending">Sincronizando ({totalPending})</span>}
        </div>
      )}
      ...
      {hasFailures && (
        <>
          ...
          <button onClick={handleRetry} disabled={retrying} className="sync-retry-btn">
            {retrying ? 'Reintentando...' : 'Tentar novamente'}
          </button>
        </>
      )}
      {!hasQueue && !hasFailures && status.completed > 0 && (
        <span className="sync-success" data-testid="sync-success">
          Sincronizado
        </span>
      )}
      <style jsx>{`
