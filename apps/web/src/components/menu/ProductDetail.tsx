...
export function ProductDetail({ productId, restaurantId, onAddToCart }: ProductDetailProps) {
  ...
  const handleModifierChange = (groupId: string, values: SelectedModifier[]) => {
    setSelectedModifiers((prev) => [...prev.filter((m) => m.group_id !== groupId), ...values]);
  };
    ...
    <div className={styles.container}>
      <div className={styles.imageSection}>
        {hasImage ? (
          ...
          <div className={styles.placeholder} style={{ background: gradientBg }} aria-hidden="true">
      ...
      <div className={styles.content}>
        <div className={styles.header}>
          <h1 className={styles.name} data-testid="product-detail-name">
            {product.name}
          </h1>
          <span className={styles.price} data-testid="product-detail-price">
            {formatPrice(product.price)}
          </span>
        </div>

        {product.description && (
          <p className={styles.description} data-testid="product-detail-description">
            {product.description}
          </p>
        )}
