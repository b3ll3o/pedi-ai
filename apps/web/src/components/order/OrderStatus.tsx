...
const STATUS_ORDER: Enum_order_status[] = [
  'pending_payment',
  'paid',
  'preparing',
  'ready',
...
export function OrderStatus({ orderId }: OrderStatusProps) {
  ...
  const isActive = (index: number) =>
    currentStatus === 'cancelled' ? false : index <= currentIndex;
      ...
      <div className="flex flex-col items-center gap-3 text-center" data-testid="order-status">
        {(() => {
          ...
          return (
            <>
              <div
                className={`relative flex h-16 w-16 items-center justify-center rounded-full bg-muted ${config.color}`}
              >
