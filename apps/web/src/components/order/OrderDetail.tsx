...
export function OrderDetail({ orderId, onReorder }: OrderDetailProps) {
  ...
  useEffect(() => {
    async function fetchOrder() {
      ...
      try {
        const response = await fetch(`/api/orders/${orderId}?restaurant_id=${restaurantId}`);
  ...
  if (error || !order) {
    return (
      <div className="flex flex-col items-center gap-4 p-6 text-center">
        ...
        <p className="text-muted-foreground">{error || 'Failed to load order details'}</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6 p-4">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`flex items-center gap-1 text-sm font-medium ${currentStatusConfig.color}`}
          >
          ...
          {order.items.map((item) => (
            <div key={item.id} className="flex items-start justify-between">
              <div className="flex flex-col">
                ...
                {item.notes && (
                  <span className="text-xs text-muted-foreground italic">Obs: {item.notes}</span>
                )}
              </div>
                ...
                return (
                  <div key={entry.id} className="relative">
                    ...
                    <div className="flex flex-col">
                      ...
                      {entry.notes && (
                        <span className="mt-1 text-sm text-muted-foreground">{entry.notes}</span>
                      )}
                    </div>
                  </div>
                );
