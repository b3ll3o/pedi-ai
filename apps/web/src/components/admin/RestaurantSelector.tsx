...
export function RestaurantSelector({
  ...
  const { restauranteSelecionado, restaurantesAcessiveis, isLoading, setRestaurante } =
    useRestaurantStore();
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case 'ArrowDown':
          event.preventDefault();
          setFocusedIndex((prev) => (prev < restaurantesAcessiveis.length - 1 ? prev + 1 : prev));
          break;
        case 'ArrowUp':
          event.preventDefault();
            ...
            <ul className={styles.dropdownList} ref={listRef}>
              {restaurantesAcessiveis.map((restaurant, index) => (
                ...
                >
                  ...
                  {!restaurant.ativo && <span className={styles.inativoBadge}>Inativo</span>}
