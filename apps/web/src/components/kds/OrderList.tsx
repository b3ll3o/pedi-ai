...
export function OrderList({
  ...
  const renderColumn = (status: keyof typeof STATUS_COLUMNS, orders: PedidoKDS[]) => {
    const columnInfo = STATUS_COLUMNS[status];
    const showAcceptButton = status === 'paid';

    return (
      <section key={status} className={styles.column} aria-labelledby={`kds-column-${status}`}>
        <header className={styles.columnHeader}>
          <h2 id={`kds-column-${status}`} className={styles.columnTitle}>
            <span aria-hidden="true">{columnInfo.icon}</span>
            {columnInfo.title}
          </h2>
          <span className={styles.count} aria-label={`${orders.length} pedidos`}>
            {orders.length}
          </span>
        </header>
        <div
    ...
    return (
      <div className={styles.container}>
        <div className={styles.columns}>
          <section className={styles.column} aria-labelledby={`kds-column-${selectedStatus}`}>
            <header className={styles.columnHeader}>
              <h2 id={`kds-column-${selectedStatus}`} className={styles.columnTitle}>
                <span aria-hidden="true">{STATUS_COLUMNS[selectedStatus]?.icon}</span>
