...
export class Restaurante extends EntityClass<RestauranteProps> {
  ...
  static criar(
    props: Omit<RestauranteProps, 'id' | 'criadoEm' | 'atualizadoEm' | 'deletedAt' | 'version'>
  ): Restaurante {
    const now = new Date();
  ...
  async pertenceAoUsuario(
    ...
    const vinculo = await repository.findByUsuarioIdAndRestauranteId(usuarioId, this.id);
    return vinculo !== null;
  }
}
