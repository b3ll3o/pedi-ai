...
import type {
  ISupabaseDatabaseAdapter,
  SyncResult,
} from '@/application/shared/services/adapters/ISupabaseDatabaseAdapter';
...
export class SupabaseDatabaseAdapter implements ISupabaseDatabaseAdapter {
  ...
  async updateRemote<T>(tableName: string, data: T[]): Promise<SyncResult> {
    ...
    const { data: result, error } = (await this.supabase
      ...
      .match({ id: (data as { id: string }[]).map((d) => d.id) })) as {
      data: unknown[] | null;
      error: unknown;
    };
  ...
  async deleteRemote(tableName: string, ids: string[]): Promise<SyncResult> {
    ...
    const { error } = await this.supabase.from(supabaseTable).delete().in('id', ids);
