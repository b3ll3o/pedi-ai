...
export default function ModifiersPage() {
  ...
  const syncModifierValues = useCallback(
    async (groupId: string, newValues: ModifierValueInput[]) => {
          ...
          if (data.values && data.values.length > 0) {
            for (const value of data.values) {
              const valueResponse = await fetch(
                `/api/admin/modifiers/${modifier_group.id}/values`,
                {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
      ...
      <div className={styles.container}>
        <header className={styles.header}>
          <div className={styles.titleSection}>
            ...
            <p className={styles.subtitle}>Gerencie os grupos de modificadores</p>
          </div>
                ...
                <div key={group.id} className={styles.item}>
                  <div className={styles.itemInfo}>
                    ...
                    <p className={styles.itemMeta}>
                      {group.required ? 'Obrigatório' : 'Opcional'} • {group.min_selections}–
                      {group.max_selections} opções
