...
async function handleDevWebhook(eventId: string, paymentId: string): Promise<NextResponse> {
  ...
  logger.info('webhooks/pix', 'Pagamento processado com sucesso (dev)', {
    orderId,
    status: orderStatus,
  });
      ...
      mpPayment.status === 'approved'
        ...
        : mpPayment.status === 'pending'
          ...
          : mpPayment.status === 'rejected' || mpPayment.status === 'cancelled'
            ? 'failed'
            : 'pending';
    await supabase.from('payment_intents').update({ status: intentStatus }).eq('order_id', orderId);
  }

  // Record webhook event for idempotency
  await supabase.from('webhook_events').insert({ id: eventId, event_type: 'payment' });
