  ...
  try {
    ...
    if (isDevDatabase()) {
      ...
      const existingOrder = await db
        ...
        .where(
          and(
            eq(orders.customer_id, body.customer_id),
            eq(orders.idempotency_key, body.idempotency_key)
          )
        )
        .limit(1);
      ...
      if (!restaurantId) {
        const restaurantResult = await db.select({ id: restaurants.id }).from(restaurants).limit(1);
