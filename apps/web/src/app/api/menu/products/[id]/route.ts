  ...
  const modifierGroupsData =
    modifierGroupIds.length > 0
      ? db
          ...
          .where(and(eq(schema.modifierGroups.restaurant_id, restaurantId)))
          .all()
...
async function handleSupabaseProductDetail(
  id: string,
  restaurantId: string
  ...
  // Fetch modifier groups
  const { data: modifierGroups, error: modifierGroupsError } =
    modifierGroupIds.length > 0
      ? await supabase
  ...
  // Fetch modifier values for these groups
  const { data: modifierValues, error: modifierValuesError } =
    modifierGroupIds.length > 0
      ? await supabase
  ...
  if (modifierValuesError) {
    ...
    return NextResponse.json({ error: 'Failed to fetch modifier values' }, { status: 500 });
  }
