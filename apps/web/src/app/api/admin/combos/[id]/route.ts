...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (isDevDatabase()) {
      ...
      const updatedCombo = await db.select().from(combos).where(eq(combos.id, id)).limit(1).get();
    ...
    if (bundle_price !== undefined && (typeof bundle_price !== 'number' || bundle_price < 0)) {
      return NextResponse.json({ error: 'bundle_price deve ser um número >= 0' }, { status: 400 });
    }
    ...
    if (product_ids !== undefined) {
      ...
      if (productsError) {
        ...
        return NextResponse.json({ error: 'Falha ao validar produtos' }, { status: 500 });
      }
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      if (updateError) {
        ...
        return NextResponse.json({ error: 'Falha ao atualizar combo' }, { status: 500 });
      }
    }
    ...
    if (product_ids !== undefined) {
      ...
      if (product_ids.length > 0) {
        ...
        const { error: itemsError } = await supabase.from('combo_items').insert(comboItemsData);

        if (itemsError) {
          console.error('Erro ao atualizar itens do combo:', itemsError);
          return NextResponse.json({ error: 'Falha ao atualizar itens do combo' }, { status: 500 });
        }
      }
    }
    ...
    if (fetchUpdatedError) {
      ...
      return NextResponse.json({ error: 'Falha ao buscar combo atualizado' }, { status: 500 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (deleteError) {
      ...
      return NextResponse.json({ error: 'Falha ao deletar combo' }, { status: 500 });
    }
